...
saveat=0.1, # Сохраняем каждые 0.1 единицы
↪ времени
dense=true # Включаем плотный вывод для
↪ интерполяции
)
# Подготовка данных
df_lv = DataFrame()
...
df_lv[!, :dprey_dt] = p_lv[1] .* df_lv.prey .- p_lv[2] .* df_lv.prey
↪ .* df_lv.predator
...
y_nullcline = p_lv[1] ./ (p_lv[2] .* x_range) # y-изоклина (dy/dt =
↪ 0)
...
↪ label=false)
# График 4: Относительные изменения (в %)df_lv[!, :prey_pct_change] = df_lv.dprey_dt ./ df_lv.prey .* 100
df_lv[!, :predator_pct_change] = df_lv.dpredator_dt ./
↪ df_lv.predator .* 100
plt4 = plot(df_lv.t, [df_lv.prey_pct_change
...
n = length(signal)
# Используем rfft для вещественных сигналов (возвращает только
...
idx_prey = argmax(spectrum_prey[2:end]) + 1 # пропускаем нулевую
↪ частоту
...
# Вместо сложного анализа сдвига фаз, просто посчитаем основные
↪ характеристики
