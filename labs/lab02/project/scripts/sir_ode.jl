...
# Доли восприимчивых, инфицированных и выздоровевших
plt4 = @df df_ode plot(:t, [:S :I :R] ./ :N,
    label=[L"S(t)/N" L"I(t)/N" L"R(t)/N"],
    xlabel="Время, дни",
    ylabel="Доля населения",
    title="Доли восприимчивых, инфицированных и выздоровевших",
    linewidth=2,
    legend=:right,
    grid=true,
    size=(800, 400),
...
plt5 = plot(df_ode.S, df_ode.I,
    ...
    xlabel=L"S(t) (Восприимчивые)",
    ylabel=L"I(t) (Инфицированные)",
    title="Фазовый портрет S-I",
    color=:blue,
    linewidth=2,
    grid=true,
    size=(800, 400))
...
hline!(plt6, [1.0], color=:red, linestyle=:dash,
    label="Порог эпидемии (Re=1)", linewidth=1.5)
...
if !isnothing(cross_idx) && cross_idx > 1
    ...
    annotate!(plt6, cross_time, 1.2,
        text("Re<1 c $(round(cross_time, digits=1)) дня", 8, :left))
end
